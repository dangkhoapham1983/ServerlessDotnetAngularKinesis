#!/usr/bin/env bash
STACK_NAME=$(xmllint ServerlessDotnetAngularKinesis.csproj --xpath "string(/Project/PropertyGroup/StackName)")
SPA_ROOT=$(xmllint ServerlessDotnetAngularKinesis.csproj --xpath "string(/Project/PropertyGroup/SpaRoot)")
S3_BUCKET_NAME=$(xmllint ServerlessDotnetAngularKinesis.csproj --xpath "string(/Project/PropertyGroup/S3BucketName)")
DEFAULT_AWS_REGION=$(xmllint ServerlessDotnetAngularKinesis.csproj --xpath "string(/Project/PropertyGroup/DefaultAWSRegion)")
INDEX_PAGE=$(xmllint ServerlessDotnetAngularKinesis.csproj --xpath "string(/Project/PropertyGroup/IndexPage)")
echo $STACK_NAME
echo ${SPA_ROOT%?}
